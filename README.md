...
The Paris Fire Brigade dataset is **not** included in this repository. It is
distributed by ENS Paris on the Challenge Data platform, which is where it
belongs — the platform holds the terms under which the brigade released it:

**https://challengedata.ens.fr/challenges/21**

Register there, download `x_train`, `x_test` and their additional files, and
place them under `data/raw/paris_fire_brigade_2018/`. The directory structure
is already in place.
...
Download the repo
```
git clone https://github.com/Ben74Builds/unit-response-oracle
...
This is the reference solution for the data challenge *Predicting response
times of the Paris Fire Brigade vehicles*, which I initiated while working at
the brigade and presented at the Collège de France, in Stéphane Mallat's Data
...
top of this README — the point of a well-posed challenge being that the
entrants beat the baseline, which they did.
